...
const NewsCard = ({title, source, publishedAt,description,urlToImage, url}) => {
    ...
    return (
        <div className="bg-[#d3d3d34d] hover:bg-gray-200 hover:scale-105 w-72 m-3 p-2 min-h-72 shadow-lg relative">
            <img src={urlToImage} className="h-28 flex m-auto" alt="img not found"/>
            ...
            <a href={url} target='_blank'><button className="bg-cyan-600 p-1 rounded-md text-white  font-semibold m-1 absolute bottom-1 left-1">Read More</button></a>
        </div>
    )
}
